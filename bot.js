...
async function handleUpdate(update) {
    if (update.message) {
        ...
        const text = msg.text || '';
        const username = msg.from && msg.from.username ? '@' + msg.from.username : 'Customer';
    ...
    if (update.message && update.message.web_app_data) {
        try {
            ...
            if (data.type === 'new_order') {
                ...
                await sendMessage(update.message.chat.id, '✅ Order Confirmed!\n\n🆔 #' + order.id + '\n💰 Total: ' + order.total + ' Birr\n🕐 ' + ts + '\n\n⏱️ Processing: 15-25 days\n💳 Pay via Telebirr:\n📱 ' + PAYMENT_PHONE + '\n👤 ' + PAYMENT_NAME + '\n🤝 Meetup after payment\n\n📞 @' + SUPPORT_USERNAME + '\n📢 ' + CHANNEL + '\n\nThank you! ✨');
...
console.log('📢 Broadcast: ON | 🔔 Notifications: ON');
